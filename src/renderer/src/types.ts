...
export interface Task {
  ...
  // 進行中 (in-progress) cumulative wall-clock tracking. Maintained centrally by the
  // UPDATE_TASK reducer: doingSince is stamped on entering 'in-progress' and folded
  // into doingMs on leaving it. doingMs alone is the settled total; while the task
  // is 'in-progress' the live total is doingMs + (now - doingSince). Both are
  // user-overridable from the task editors (手修正).
  doingMs?: number // accumulated milliseconds spent in 'in-progress' (settled segments only)
  doingSince?: string // ISO timestamp of the current 'in-progress' segment start (set iff status === 'in-progress')
